...
public class Playerhand {
	...
	public Playerhand(String temp, int index, ClientInterface ci) {
		mygui = ci;
		name = temp;
		current_phase = 0;
		phaseTextIndex = index;
//		mygui.phaseText.add(phaseTextIndex, "Phase " + (current_phase + 1)
//				+ " for " + name + ":\nNot completed");
//		mygui.updatePhaseHistory();
	...
	public Card getCard(int index) {
		Card tempCard = new Card(player_hand.get(index).getValue(), player_hand
				.get(index).getColor());
		return tempCard;
	}
	...
	public void printhand() {
		...
		for (int i = 0; i < player_hand.size(); i++) {
			player_hand.get(i).printCard(mygui);
		}
	}
	...
	public void printPhasehand() {
		...
		for (int i = 0; i < phase_hand.size(); i++) {
			tempPhasestring = tempPhasestring + phase_hand.get(i).getColor()
					+ phase_hand.get(i).getValue() + " ";
		}
		mygui.phaseText.add(phaseTextIndex, "Phase " + (current_phase)
				+ " for " + name + ":\n" + tempPhasestring);
		mygui.updatePhaseHistory();
	}
	*/
...
//		printPhasehand();
	...
	public boolean phaseCheck() {
		if (current_phase == 0) {
			...
			boolean wild = false;
			for (int i = 0; i < player_hand.size(); i++) {
				if (player_hand.get(i).getValue() != 14
						&& player_hand.get(i).getValue() != 13
						&& player_hand.get(i).getValue() != first_set) {
					for (int j = 0; j < player_hand.size(); j++) {
						if (player_hand.get(i).getValue() == player_hand.get(j)
								.getValue() && i != j)
							threeOfaKind++;
						...
						System.out.println(player_hand.get(i).getValue() + " "
								+ threeOfaKind);
					...
					if (threeOfaKind >= 3) {
						...
						wild = ClientInterface.c.useWild(1, player_hand.get(i)
								.getValue());
						if (wild == true) {
							set++;
							if (first_set == 0)
					...
					if (set == 2) {
						...
						System.out.println("My cards are: " + cardValue1 + " "
								+ cardValue2);
